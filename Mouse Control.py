...
#range for HSV (yellow color)
low_g=np.array([130,100,100])
up_gg=np.array([170,255,255])
...
while True:
    ...
    mask=cv2.inRange(imgHSV,low_g,up_gg)

    #using morphology to erase noise as maximum as possible 
    n_mask=cv2.morphologyEx(mask,cv2.MORPH_OPEN,kernelOpen)
    another_mask=cv2.morphologyEx(n_mask,cv2.MORPH_CLOSE,kernelClose)
    ...
    if(len(conts)==2):
        ...
        fx2=int(x2+w2/2)
        fy2=int(y2+h2/2)
        cv2.line(img,(fx1,fy1),(fx2,fy2),(255,0,0),2)
    ...
    #when there's only when object detected it will act as a left click mouse    
        ...
        if(IsPress==0):
            ...
            if(abs((w*h-openw*openh)*100/(w*h))<30): #the difference between th combined rectangle for both objct and the 
                IsPress=1                          #the outer rectangle is not more than 30%
